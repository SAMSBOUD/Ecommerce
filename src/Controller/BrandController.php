...
class BrandController extends AbstractController
{
    ...
    #[Route("/admin/api/categories/{productId}", name:"admin_api_categories")]
     
    public function getCategoriesByProduct($productId, BrandRepository $brandRepository): JsonResponse
    {
        $brands = $brandRepository->findByBrandByCategorie($productId);

        $data = [];
        foreach ($brands as $brand) {
            $data[] = [
                'id' => $brand->getId(),
                'name' => $brand->getLibelle(),
            ];
        }
